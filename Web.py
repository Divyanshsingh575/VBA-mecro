...
# Define the full palette colors
palette_colors = ["#3a3a3a", "#e53935", "#fb8c00", "#fdd835", "#39cb7f", "#42a5f5"]
...
def check_environment_functions(vba_code):
    patterns = [
        ...
        r"Environ\(\"windir\"\)",  # Getting Windows directory
        ...
        r"Environ\(\"PATHEXT\"\)",  # Getting executable file extensions
...
def main():
    # Create columns for the color bars using the full palette
    cols = st.columns(len(palette_colors))
    for idx, color in enumerate(palette_colors):
        if idx < len(cols):
            cols[idx].markdown(f"""<div style="background-color: {color}; height: 40px; margin-right: -20px;"></div>""",
                               unsafe_allow_html=True)
    ...
    st.markdown(
        f'<div style="text-align:center"><img src="data:image/jpeg;base64,{image_data_base64}" width="265"></div>',
        unsafe_allow_html=True)

    # Add the heading
    st.markdown("<h1 style='text-align:center;color:#3a3a3a;'>Worried about macros status in a received file?</h1>",  # dark grey
                unsafe_allow_html=True)
    ...
    # Note centered
    st.markdown("<p style='text-align:center;'><strong style='color:green;'>*Note:</strong> <em><strong>Only Microsoft Office PowerPoint, Word, and Excel are accepted.</strong></em></p>",
                unsafe_allow_html=True)

    file_path = st.file_uploader("Select analyze:",
                                 type=['xls', 'xlsm', 'xlsb', 'xlsx', 'docx', 'docm', 'pptx', 'pptm'])
    if file_path:
